...
Pinning the dev-repo is optional but recommended.
...
If `opam pin <package-name> --dev-repo` does not work, you can also try:
...
build it manually as described in the [documentation](https://ergo-code.github.io/HiGHS/dev/installation/).
...
In this case, `HIGHS_INCLUDE_DIR` / `HIGHS_LIB_DIR` are usually not necessary.
...
A more detailed description can be found on the [wiki](https://github.com/ktahar/ocaml-lp/wiki/).
...
High-level APIs include comments for odoc (or ocamldoc).
Generated docs can be found [online](https://ktahar.github.io/ocaml-lp/) or in the [docs](docs) directory.

## Development Status

Development is not very active now because the basic features are complete.
...
Currently, only the basic features of the LP file format are supported.
Advanced features, which are typically available on commercial solvers, are not yet supported.
(There is no standard LP file format, though.)
